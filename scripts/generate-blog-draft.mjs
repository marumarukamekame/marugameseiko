...
const requestedCategory = process.env.BLOG_CATEGORY || "地域の健康";
if (!categories.some(({ name }) => name === requestedCategory)) throw new Error(`Unknown BLOG_CATEGORY: ${requestedCategory}`);
const today = new Date().toISOString().slice(0, 10);
const topic = process.env.BLOG_TOPIC || `${requestedCategory}について横浜の地域住民が日常で実践できること`;
const schema = {
  type: "object", additionalProperties: false,
  required: ["slug", "title", "description", "imagePrompt", "introduction", "sections", "sources"],
  properties: {
    slug: { type: "string", pattern: "^[a-z0-9-]+$" }, title: { type: "string" }, description: { type: "string" }, imagePrompt: { type: "string" }, introduction: { type: "string" },
...
const draft = { slug: `${today.slice(0, 7)}-${generated.slug}`.slice(0, 80), status: "draft", title: generated.title, description: generated.description, publishedAt: null, updatedAt: today, category: requestedCategory, image: "/marugameseiko/assets/images/blog/walking-placeholder.svg", imageAlt: `${generated.title}のアイキャッチ画像（準備中）`, imagePrompt: generated.imagePrompt, introduction: generated.introduction, sections: generated.sections, sources: verifiedSources, relatedActivities: [], relatedPosts: [] };
