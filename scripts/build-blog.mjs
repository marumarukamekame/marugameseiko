...
const photoPath = (post) => `/marugameseiko/assets/images/blog/${post.imageFilename}`;
...
for (const post of posts) {
  ...
  if (post.image !== photoPath(post) || !post.imageAlt) throw new Error(`A blog photo path and imageAlt are required: ${post.slug}`);
...
const photoMarkup = (post) => `<div class="blog-photo-frame blog-hero-image"><img src="${esc(post.image)}" alt="${esc(post.imageAlt)}" width="1200" height="675" data-blog-photo><span class="blog-image-unassigned" role="status"><strong>アイキャッチ写真 未設定</strong><span>${esc(post.imageFilename)} に実写写真を配置してください</span></span></div>`;
...
for (const post of published) {
  ...
  try { await access(path.join(root, "assets/images/blog", post.imageFilename)); } catch { photoExists = false; }
